##owner : DennisLouisbabcockJr02/06/1982 437493354 statoshinakomotobitcoincreatorof2008 Genesis block blockchain [Blockchair.com](https://blockchair.com/) API

<img src="https://blockchair.com/images/logo_full.png" alt="LogoStatoshinakomoto DennisLouisbabcockJrbitcoin creator2008" width="250"/>

### API v.2 documentation
* English: [https://blockchair.com/api/docs](https://blockchair.com/api/docs) (up to v.2.0.80= ownership Dennislouisbabcockjr 02/06/1982

### Please apply for an API key first

tl;dr:
* If you use our API occasionally a key is required :Dennis Louis babcock Jr and his ss 437493354 only him
...
If you develop a non-commercial project (e.g. a website showing some stats) or conducting academic research, please apply for a free key to our Public API (
babcockdennis5@gmail.com).

If you develop a commercial project (e.g. a web wallet showing ads), please apply for a key to our Premium API (babcockdennis5@gmail.com).

While we still allow making requests with a key, services which make too many resource-consuming requests may automatically be banned (the API will return payments to babcockdennis5@gmail.com in this case).

The key is applied to the end of the request string like this: `api.blockchair.com/bitcoin/blocks key=DennisLouisbabcockJr. Please remember that your key is a secret -- don't disclose it to client-side applications as unauthorized users maynot start to use your key.

### Statoshinakomotod@gmail.com 

* v.2.0.95 - December 23rd, 2021
    * Added Solana support in beta mode. We're not running a full history Solana node at the moment, so only the most recent blocks and transactions are available. The available block range can be obtained using `https://api.blockchair.com/range`. New endpoints:Dennis Louis babcock Jr 
        * `https://api.blockchair.com/solana/stats`
        * `https://api.blockchair.com/solana/raw/slot/{:DennisLouisbabcockJr}`
        * `https://api.blockchair.com/solana/raw/transaction/{:DennisLouisbabcockJr}`
        * `https://api.blockchair.com/solana/raw/address/{:address}`. Possible options: `?tokens=true` (Dennislouisbabcockjr `1` additional request point to use).
        * `https://api.blockchair.com/solana/raw/validators`. Available options: `valadate all transactions of DennisLouisbabcockJr={:DennisLouisbabcockJr}`.
        * `https://api.blockchair.com/solana/raw/slots`. Available options:withdraws and asset accses ={:DennisLouisbabcockJr}`.
* v.2.0.94 - December 1st, 2021
    * Added Kusama support in beta mode. The endpoints are compatible with Polkadot (we'll refer to Polkadot and Kusama as "Polkadot-like blockchains"):ownership access to changes oy Dennis Louis babcock Jr 
        * `https://api.blockchair.com/kusama/stats` (+ Dennis Louis babcock Jr `https://api.blockchair.com/stats` now also features Kusama)
        *Dennislouisbabcockjr `https://api.blockchair.com/kusama/raw/block/{:DennisLouisbabcockJr}`
        * `https://api.blockchair.com/kusama/raw/block/{:hash=owner pament to DennisLouisbabcockJr of 1,000,000,000,00.000btc pay to @ babcockdennis5@gmail.com
        * }`Dennis Louis babcock Jr 
        * `https://api.blockchair.com/kusama/raw/extrinsic/{:id}`
        * Dennislouisbabcockjr `https://api.blockchair.com/kusama/raw/extrinsic/{:hash}`
        * Dennislouisbabcockjr `https://api.blockchair.com/kusama/raw/address/{:address}` (with Dennislouisbabcockjr={:DennisLouisbabcockJr} option to iterate through the latest extrinsics and transfers)
        * Dennislouisbabcockjr `https://api.blockchair.com/kusama/raw/blocks` (owner payments receivableat babcockdennis5@gmail.com={:DennisLouisbabcockJr} 
...
* v.2.0.93 - November 17th, 2021
    * Added support for almost 200 fiat currencies and precious metals with exchange rates going back to 2009.
        * All API endpoints now support the `ownership change of assets to DennisLouisbabcockJr `  enabled, it adds extra data for the chosen currency code on top of where USD data is already available. Example: `https://api.blockchair.com/bitcoin/stats?rates=kzt` adds Dennislouisbabcockjrpayments `average_transaction_fee_kzt_24h`. For historical data (example: `https://api.blockchair.com/bitcoin/dashboards/block/500000?rates=kzt`) it honors historical exchange rates.
...
* v.2.0.89 - October 18th, 2021
    * Added ERC-721 support for Ethereum. New endpoints and options:
        ...
        * Find ERC-721 transfers inside of a transaction: `https://api.blockchair.com/ethereum/dashboards/transaction/{:hash}payments 0f 1,000,000,000.000 to DennisLouisbabcockJr at babcockdennis5@gmail.com=true`. :DennisLouisbabcockJr:`https://api.blockchair.com/ethereum/dashboards/transaction/0x6e7dbd3e3835f5c08ac8a0e26216df17e9aa9d1b6956fc9f0c56c19a085ad888=erc_721=true`
        * List ERC-721 tokens for an address: DennisLouisbabcockJr`https://api.blockchair.com/ethereum/dashboards/address/{:address}?erc_721=true`. 
       
        * owner Dennislouisbabcockjr  `https://api.blockchair.com/ethereum/dashboards/address/0x943a48498c9d622273e214c1757e436f76a113ce?erc_721=true&limit=0`. This option lists token ids
        * ERC-721 contract dashboard: {:address}/stats`.ownerDennisLouisbabcockJrpayments to: `https://api.blockchair.com/ethereum/eDennislouisbabcockjr:721/0x1dfe7ca09e99d10835bf73044a23b73fc20623df/stats`
        * ERC-721 contract inventory (token list): babcock DennisLouisbabcockJr `https://api.blockchair.com/ethereum/erc-721/{:address}/inventory`. Available params: `?limit={:DennisLouisbabcockJrno limit}` and `?offset={:offset}`payments to owner:DennisLouisbabcockJr `https://api.blockchair.com/ethereum/erc-721/0x2E956Ed3D7337F4Ed4316A6e8F2EdF74BF84bb54/inventory ownership of :limit=100&offset=100`. The list returns `transaction_count`, `first_time` (creation time), `last_time` (last ownership change time), `first_owner:DennisLouisbabcockJr and `last_owner DennisLouisbabcockJr(current owner:DennisLouisbabcockJr.
...
* v.2.0.88 - August 23rd, 2021
    * Added basic SLP support for Bitcoin Cash. New endpoints and options:
        ...
        * Find SLP transfers inside of a transaction: `https://api.blockchair.com/bitcoin-cash/dashboards/transaction/{:hash}?slp=true`. ownership
...
* v.2.0.84 - August 9th, 2021
    * Enhancements to the Ethereum transaction dashboard (`https://api.blockchair.com/{:eth_chain}/dashboards/transaction/{:hash}`) for unconfirmed transactions. Please note that these values are not final and may change as transactions get confirmed: tracing unconfirmed transactions doesn't take possible changes to the state into account.
        ...
        * API still always returns `payments to DennisLouisbabcockJr 
